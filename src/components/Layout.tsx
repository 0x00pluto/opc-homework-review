  ...
  const navGroups = [
    {
      ...
      items: [
        ...
        { name: "工作量统计", href: "/workload", icon: BarChart3 },
      ],
    },
    {
      title: "教务配置管理",
...
const studentNavItems = [
  { name: "作业中心", href: "/", icon: LayoutDashboard },
  { name: "成长轨迹", href: "/growth", icon: LineChart },
];
...
export function StudentSidebar() {
  ...
  const displayName = profile?.name || userId || "学员";
    ...
    <aside className="w-64 bg-white border-r border-slate-200 flex flex-col shrink-0">
      <div className="p-6 border-b border-slate-200 flex items-center gap-3">
        ...
        <div className="flex flex-col min-w-0">
          ...
          {profile?.cohort_name && (
            <span className="text-[10px] text-indigo-600 font-medium truncate mt-0.5">
              {profile.cohort_name}
            </span>
          )}
        </div>
      ...
      <div className="p-4 border-t border-slate-200">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 rounded-lg transition-colors border border-transparent hover:border-slate-200"
        >
          <LogOut className="w-4 h-4" /> 退出学员端
        </button>
      </div>
    </aside>
  );
        ...
        {role === "student" && (
          <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 flex-shrink-0 z-10 relative shadow-sm">
            <div className="flex items-center gap-2 text-sm font-medium text-slate-500">
              ...
              <span className="text-slate-300">/</span>
              <span className="text-slate-900 font-bold">学员工作台</span>
            </div>
            <div className="flex gap-3">
